...
int set_interface_attribs (int fd, int speed, int parity)
{
  ...
  if (ioctl(fd, TCGETS2, &tty) != 0) {
    fprintf(stderr, "Cannot get device attributes\n");
    exit(1);
  }
  ...
  tty.c_cc[VMIN]  = 0;            // read atleast 2 bytes
...
void dtmxsdb_t::start_host_thread()
{
  ...
  set_interface_attribs (sock, 1000000, 0);  // set speed to 115,200 bps, 8n1 (no parity)
