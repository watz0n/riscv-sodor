...
Sodor is also ported to FPGA([PYNQ-Z1](http://store.digilentinc.com/pynq-z1-python-productivity-for-zynq/)) and [ARTY](https://store.digilentinc.com/arty-a7-artix-7-fpga-development-board-for-makers-and-hobbyists/) wherein 3stage core is used 
along with the uncore from Rocket-Chip making it capable to speak [tilelink](https://static.dev.sifive.com/docs/tilelink/tilelink-spec-1.7-draft.pdf) 
and AXI4(via tilelink adapter). It is able to execute and pass all riscv-tests and riscv-bmarks on FPGA. Currently there are no caches and DDR on FPGA is 
the only accessible storage. ELF's are loaded via [XSDB](https://www.xilinx.com/support/documentation/sw_manuals/xilinx2014_3/SDK_Doc/concepts/sdk_c_xsd_over.htm), 
for more details check [README](emulator/fpgazynq/README.md) for PYNQ-Z1. For Arty ELF's are loaded using [glip](glip.io)/uart. Apart from emulating sodor
 on PYNQ-Z1, the surrounding that 3stage faces on PYNQ-Z1, can also be simulated in software [zynqsimtop](emulator/zynqsimtop). 
For more details try microarchitecture diagrams in /doc 
...
See doc/ for microarchitecture diagrams which can be viewed using [draw.io](https://www.draw.io) using the following example link https://www.draw.io/?url=https://raw.githubusercontent.com/librecores/riscv-sodor/master/doc/fpga.xml with master/doc/fpga.xml to be changed as desired
...
optionally, the [spike-dasm](https://github.com/riscv/riscv-isa-run) for the binary to
...
To compile your own tests, you will need to build an
...
To know more about how the binaries are loaded into processors memory - [Overview](https://github.com/librecores/riscv-sodor/wiki/overview) 
